  ...
  generate(img, o) {
    ...
    for(let y = 0; y < img.h; y++) {
      for(let x = 0; x < img.w; x++) {
        ...
        /*let xN = (x / (img.w - 1) - 0.5) * 2;
        let yN = (y / (img.h - 1) - 0.5) * 2;
        const r = Math.sqrt(xN * xN + yN * yN);
        //V the problem
        let theta = Math.atan2(xN, yN) - angleOffs;
        //theta += t * 0.005 * Math.PI;
        xN = Math.sin(theta) * r;
        yN = Math.cos(theta) * r;
        const xRot = (xN / 2 + 0.5) * img.w;
        const yRot = (yN / 2 + 0.5) * img.h;
        //const offsetIdx = mod(y + Math.floor(yOffs * magnitude), img.h) * img.w;
        //const col = img.getPixelFromData(clamp(xRot, 0, img.w - 1), clamp(yRot, 0, img.h - 1), baseData, o.interpMode);
        ...
        const xNew = x - pivotX;
        const yNew = y - pivotY;
        const xRot = Math.cos(angleOffs) * xNew - Math.sin(angleOffs) * yNew;
        const yRot = Math.sin(angleOffs) * xNew + Math.cos(angleOffs) * yNew;
        //const col = img.getPixelFromData(mod(xRot, img.w), mod(yRot, img.h), baseData, o.interpMode);
